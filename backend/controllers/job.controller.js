...
export const postJob = async (req, res) => {
    try {
        ...
        const experienceLevelNum = Number(experience);
        ...
        if (isNaN(experienceLevelNum) || experienceLevelNum < 0) {
            return res.status(400).json({
                message: "Experience level must be a valid non-negative number.",
                success: false
            });
        }
...
export const updateJob = async (req, res) => {
    try {
        ...
        if (requirements) updateData.requirements = typeof requirements === 'string' ? requirements.split(",") : requirements;
